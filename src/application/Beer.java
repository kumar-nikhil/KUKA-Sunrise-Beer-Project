...
//		glassLean = pourBase.getChild("GlassJig").getChild("leanPosition");
	...
	private void getGlass() throws Exception {
		...
		targetAir.transform(tcpGrip, Transformation.ofDeg(250, 0, 0, 0, 0, 30));
		Frame glassJigAir = pourBase.copyWithRedundancy();
		glassJigAir.transform(World.Current.getRootFrame(), Transformation.ofTranslation(0,0,250));
...
//		throw Exception;
	...
	private int evaluateLoad(double innateForce) {
		...
		getLogger().info("Calculated force is : " + Math.abs(zForce-innateForce+2.0) + " N");
		double load = (zForce-innateForce+2.0) / 9.8;
